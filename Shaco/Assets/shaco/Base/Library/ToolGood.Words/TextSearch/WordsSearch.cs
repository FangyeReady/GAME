    ...
    public class WordsSearch
    {
            ...
            for (int i = 0; i < text.Length; i++) {
                ...
                if (tn != null) {
                    if (tn.End) {
                        ...
                        return new WordsSearchResult(item.text, i + 1 - item.text.Length, i, item.index);
                    }
                }
                ptr = tn;
            }
                ...
                if (tn != null) {
                    if (tn.End) {
                        foreach (var item in tn.Results) {
                            list.Add(new WordsSearchResult(item.text, i + 1 - item.text.Length, i, item.index));
                        }
                    }
                }
                ptr = tn;
        ...
        public string Replace(string text, char replaceChar = '*')
        {
            ...
            for (int i = 0; i < text.Length; i++) {
                ...
                if (tn != null) {
                    if (tn.End) {
                        var maxLength = tn.Results[0].text.Length;
