...
#if !XLUA_GENERAL
        [PostProcessScene]
        [MenuItem("shaco/XLua/Hotfix Inject In Editor", false, 3)]
...
#endif
            var inject_tool_path = shaco.Base.FileHelper.GetCurrentSourceFolderPath() + "../../Tools/XLuaHotfixInject.exe";
            if (!File.Exists(inject_tool_path))
            {
                UnityEngine.Debug.LogError("please install the Tools, inject_tool_path=" + inject_tool_path);
                return;
            }
