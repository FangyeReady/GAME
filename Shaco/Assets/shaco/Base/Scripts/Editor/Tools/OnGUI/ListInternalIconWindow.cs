...
namespace shacoEditor
{
    public class ListInternalIconWindow : EditorWindow
    {
        ...
        [MenuItem("shaco/Editor/Open Internal Icon Window")]
        static void Open()
        {
        ...
        private void UpdateWindowsIconList()
        {
            ...
            {
                if (!shaco.Base.FileHelper.ExistsDirectory(_unityEditorPath))
                {
                    Debug.LogError("ListInternalIconWindow UpdateWindowsIconList error: not found directory=" + _unityEditorPath);
                    return;
                }
            }
        ...
        void OnGUI()
        {
            ...
            {
                GUI.changed = false;
                _unityEditorPath = GUILayoutHelper.PathField("UnityEditor Directory Path", _unityEditorPath, string.Empty);
                if (GUI.changed)
                {
                    UpdateWindowsIconList();
                }
        ...
        private IEnumerator MethodParamEnumerator(string methodName, MethodInfo loadTextureMethodInfo)
        {
            ...
            string[] files = Directory.GetFiles(_unityEditorPath, "*.cs", SearchOption.AllDirectories);
        ...
        {
            shaco.DataSave.Instance.Write("ListInternalIconWindow+_unityEditorPath", _unityEditorPath);
        }

        private void LoadSettings()
        {
            _unityEditorPath = shaco.DataSave.Instance.ReadString("ListInternalIconWindow+_unityEditorPath");
        }
    }
}