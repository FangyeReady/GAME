...
namespace shaco.Base
{
    ...
    public class EventCallBack
    {
        ...
        /// <summary>
        /// Add delegate callback, If the same delegate already exists, no delegate are added again
        /// </summary>
        ...
        private void InvokeAllCallBackBase(object sender)
        {
            lock (_callBacks)
            {
                ...
                for (int i = 0; i < _callBacks.Count; ++i)
                {
                    if (null == _callBacks)
                    {
                        Log.Error("EventCallBack invokeAllCallBack erorr: _callBacks is null !");
                        break;
                    }
                    if (i >= _callBacks.Count)
                    {
                        Log.Error("EventCallBack invokeAllCallBack erorr: index out of range ! index=" + i + " callback count=" + _callBacks.Count);
                        break;
                    }
                    ...
                    {
                        Log.Error("EventCallBack invokeAllCallBack erorr: callback information is null !");
                        continue;
                    }
                    ...
                    {
                        Log.Warning("EventCallBack invokeAllCallBack warning: callback function is missing" + GetCallBackInformation(callbackInfoTmp.CallFunc));
                    }
