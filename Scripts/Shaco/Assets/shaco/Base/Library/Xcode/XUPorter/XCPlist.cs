	...
	public class XCPlist
	{
		...
		public void AddPlistItems(string key, object value, Dictionary<string, object> dict)
		{
			...
			else
			{
				...
				{
                    var listTmp = (ArrayList)value;
                    if (listTmp.Count == 0)
                    {
                        Debug.LogError("AddPlistItems error: Item is empty, key=" + key + " value=" + value);
                        return;
                    }

                    if (listTmp[0] is Hashtable)
                    {
                        dict[key] = HashtableToDictionary<string, object>((Hashtable)listTmp[0]);
                    }
                    else
                    {
                        dict[key] = listTmp;
