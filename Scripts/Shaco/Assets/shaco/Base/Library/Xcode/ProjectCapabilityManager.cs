...
namespace shaco.iOS.Xcode
{
    ...
    public class ProjectCapabilityManager
    {
        ...
        public void WriteToFile()
        {
            ...
            {
                var entitlementsPath = PBXPath.Combine(m_BuildPath, m_EntitlementFilePath);
                if (File.Exists(entitlementsPath)) File.Delete(entitlementsPath);
                m_Entitlements.WriteToFile(entitlementsPath);
            }
            if (m_InfoPlist != null)
        ...
        public void AddWallet(string[] passSubset)
        {
            ...
            {
                arr.values.Add(new PlistElementString(WalletEntitlements.AutoValue + WalletEntitlements.AutoValue));
            }
            else
            {
                ...
                {
                    if (arr != null)
                        arr.values.Add(new PlistElementString(WalletEntitlements.AutoValue + passSubset[i]));
                }
            }
        ...
        private PlistDocument GetOrCreateEntitlementDoc()
        {
            if (m_Entitlements == null)
            {
                ...
                var fileName = Path.GetFileName(m_EntitlementFilePath);
                var pathFolder = Path.Combine(m_BuildPath, m_EntitlementFilePath.Remove(fileName));
                string[] entitlementsFiles = Directory.GetFiles(pathFolder, fileName);
    ...
    internal class WalletEntitlements
    {
        ...
        internal static readonly string AutoValue = "$(TeamIdentifierPrefix)";
