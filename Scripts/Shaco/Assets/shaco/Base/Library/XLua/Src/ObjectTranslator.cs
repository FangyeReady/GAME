...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
        ...
        Delegate getDelegate(DelegateBridgeBase bridge, Type delegateType)
        {
            ...
            {
                var methodTmp = methods[i];

                if (!methodTmp.IsConstructor && Utils.IsParamsMatch(delegateMethod, methodTmp, methodTmp.IsConstructor))
                {
#if !UNITY_WSA || UNITY_EDITOR
                    return Delegate.CreateDelegate(delegateType, bridge, methodTmp);
#else
                    return methodTmp.CreateDelegate(delegateType, bridge); 
#endif
                }
            }
